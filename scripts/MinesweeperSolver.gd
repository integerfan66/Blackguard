...
##
## Performance tip:
##   Set zero_guess_mode = true (the default) when you only need a pass/fail
##   solvability check. The solver exits immediately on the first unavoidable
##   guess without doing any recursive branch exploration — O(cells) instead
##   of O(2^candidates). MineGenerator uses this automatically.
##   Set zero_guess_mode = false to count the minimum number of guesses needed,
##   which is much slower but gives richer SolveResult data for debugging.
...
## When true, abort at the first unavoidable guess (fast pass/fail check).
## When false, count minimum guesses via full recursive branch exploration.
var zero_guess_mode: bool = true

## Maximum recursion depth. Only applies when zero_guess_mode = false.
var max_guess_depth: int = 3


## Returned by analyze(). Check .is_solvable and .guesses_needed.
class SolveResult:
	var is_solvable: bool = false
	var guesses_needed: int = 0
	var guess_positions: Array[Vector2i] = []
...
func _solve(state: Dictionary, mine_set: Dictionary, log: Array, depth: int) -> int:
	...
	# Propagate until no further deductions are possible
	var progress := true
	while progress:
		if DEBUG_SOLVER:
			print("depth=%d" % depth)
	...
	var candidates := _get_guess_candidates(state, depth)
	...
	# ── Zero-guess fast path ──────────────────────────────────────────────────
	# Propagation is exhausted and unknowns remain — a guess is required.
	# In zero_guess_mode we report this immediately without exploring branches,
	# turning an exponential search into a single linear propagation pass.
	if zero_guess_mode:
		log.append({"type": "guess", "pos": candidates[0], "depth": depth})
		return 1

	# ── Full branch exploration (zero_guess_mode = false) ─────────────────────
	var best_cost: int = INF_INT
	var best_log: Array = []

	for candidate in candidates:
		# Branch A: assume this cell is SAFE
		...
		# Both branches must be survivable; take the worse of the two.
		# A guess only "costs nothing" if both outcomes lead to a solution.
		...
		if worst < best_cost:
			...
			best_log = log_safe
			if best_cost == 0:
				break  # optimal found — stop searching
...
func _propagate(state: Dictionary, mine_set: Dictionary, log: Array) -> bool:
	...
	# Collect safe cells first to avoid mutating cells[] mid-iteration
	var to_reveal: Array[Vector2i] = []
	for i in cells.size():
		if cells[i] == STATE_SAFE:
			to_reveal.append(Vector2i(i % w, i / w))
	...
	for y in h:
		for x in w:
			...
			for n in neighbors:
				var nidx := n.y * w + n.x
				match cells[nidx]:
					STATE_MINE:    flagged_count += 1
					STATE_UNKNOWN: unknown_neighbors.append(n)

			var remaining := number - flagged_count

			# Rule 1: remaining mines == unknown count → all unknowns are mines
			...
			# Rule 2: remaining == 0 → all unknowns are safe
...
## derive new mine/safe deductions on the difference set.
func _subset_rule(state: Dictionary, mine_set: Dictionary, log: Array) -> bool:
	...
	for y in h:
		for x in w:
			...
			if unknowns.size() > 0:
				# Pre-build set once per constraint for O(1) lookup in inner loop
				var unknowns_set: Dictionary = {}
				for u in unknowns:
					unknowns_set[u] = true
				constraints.append({
					"remaining": remaining,
					"unknowns": unknowns,
					"unknowns_set": unknowns_set
				})
	...
	for i in constraints.size():
		for j in constraints.size():
			...
			if not _is_subset_with_set(a["unknowns"], b["unknowns_set"]):
				continue

			var diff_remaining: int = b["remaining"] - a["remaining"]
			var diff_unknowns: Array[Vector2i] = _set_difference_with_set(b["unknowns"], a["unknowns_set"])
...
## Returns border unknown cells (adjacent to at least one revealed cell).
## In zero_guess_mode, returns all candidates unmodified — the first one is all
## we need since we exit immediately. In full mode, limits branching factor
## exponentially by depth to prevent combinatorial blowup.
func _get_guess_candidates(state: Dictionary, depth: int = 0) -> Array[Vector2i]:
	var w: int = state["width"]
	var h: int = state["height"]
	var cells: Array = state["cells"]
		...
		for x in w:
			...
			for n in _get_neighbors(Vector2i(x, y), w, h):
				if cells[n.y * w + n.x] == STATE_UNKNOWN and not seen.has(n):
					candidates.append(n)
					seen[n] = true
					# In zero_guess_mode we only need one candidate — early exit
					if zero_guess_mode:
						return candidates

	# Full mode: limit branching factor — 8 at depth 0, halving each level
	var limit := maxi(1, 8 >> depth)
...
func _is_subset_with_set(a: Array, b_set: Dictionary) -> bool:
	for item in a:
		if not b_set.has(item):
			return false
	return true


func _set_difference_with_set(a: Array, b_set: Dictionary) -> Array[Vector2i]:
	var result: Array[Vector2i] = []
	for item in a:
		if not b_set.has(item):
			result.append(item)
